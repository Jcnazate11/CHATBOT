...
def load_model(device_type, model_id, model_basename, LOGGING):
    if model_basename == "":
        # Si model_basename está vacío, asumimos que es un modelo completo
        return load_full_model(model_id, model_basename, device_type, LOGGING)
    
    if model_basename.endswith('.gguf') or model_basename.endswith('.ggml'):
        return load_quantized_model_gguf_ggml(model_id, model_basename, device_type, LOGGING)
    elif 'gptq' in model_basename.lower():
        return load_quantized_model_qptq(model_id, model_basename, device_type, LOGGING)
    elif 'awq' in model_basename.lower():
        return load_quantized_model_awq(model_id, LOGGING)
    else:
        return load_full_model(model_id, model_basename, device_type, LOGGING)
